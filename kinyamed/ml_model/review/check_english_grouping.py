#!/usr/bin/env python
"""Run the project's phrase-grouping rule over the English candidates.

Replaces `review/prefix_threshold.py`, which measured `PREFIX_UNION_CHARS` per
language. **That question was dissolved rather than answered**: the threshold was
removed from `split_dataset.py` on 2026-09-05 after measurement showed it made
ten prefix-only unions over 163 Kinyarwanda phrases, one right and eight wrong.
The English half of that analysis said keep 30; the Kinyarwanda half said the
mechanism was folding the grammar it was set above, and the mechanism lost.

What matters now is whether the replacement — ordered subsequence — behaves
differently in English, and it might: English packs meaning into function words
that recur everywhere ("I have a", "and my"), where Kinyarwanda packs it into
inflected verbs that do not. A short English phrase is a likelier subsequence of
a long one than a short Kinyarwanda phrase is.

    python review/check_english_grouping.py

Run it after every batch. It imports the real rule, so it cannot drift from what
the splitter does.
"""
...
BRIEF = ROOT / "review" / "speaker_brief_english_v2.csv"
...
def main() -> int:
    ...
    candidates = {f"{r['concept_id']}/{r['person']}": r["suggested_english"].strip()
                  for r in rows
                  if r["suggested_english"].strip() and r["applies"] != "no"}
    ...
    print(f"{len(pool)} English phrases in the v2 inventory so far")

...
def main() -> int:
    ...
    def na(r: dict) -> bool:
        return (r.get("applies") or "yes").strip().lower() == "no"

    def filled(r: dict) -> bool:
        return bool((r.get(args.column) or "").strip()) or na(r)
    ...
    print(
        f"  {bar(done, total)}  {done}/{total}  ({done / total:.0%})"
    ...
    for dom in sorted(per):
        ...
        flag = (
            "  <- next"
            if left and all(per[x][1] == 0 for x in sorted(per) if x < dom)
            else ""
        )
    ...
    try:
        ...
        auth = [
            (r, c)
            for r, c in classified(args.brief)
        ...
        if auth:
            ...
            print(
                f"    speaker's own words {own}/{n_auth} = {100 * own / n_auth:.0f}%"
                f"   |   newly composed {fresh}/{n_auth} = {100 * fresh / n_auth:.0f}%"
            )
    except Exception as exc:  # noqa: BLE001 — a report must never block the count

#!/usr/bin/env python
"""Apply the MODEL's register review to the English brief, and label it as such.

Deliberately a separate tool from `apply_english_drafts.py`, which refuses to
...
someone with standing to answer it. Neither the drafter nor the reviewer can
answer it for Rwandan English. This tool records the drafter's own judgement
because a labelled model review is worth more than no review, and for no other
reason. It is not a substitute for the verdict it fills in.

    source = machine_reviewed    drafted by a model, reviewed by the same model,
                                 NOT verified by a native or Rwandan English
                                 speaker

Rows carrying a doubt specific to Rwandan English get `rw_english_check` set, and
those are the ones that matter: they are the list a Rwandan English speaker would
work through, and they are the honest output of a review that cannot settle them.

    python review/model_register_review.py review/reviews/<file>.csv
    python review/model_register_review.py <file>.csv --dry-run
"""

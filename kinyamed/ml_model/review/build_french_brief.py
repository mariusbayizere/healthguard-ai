...
def build() -> list[dict]:
    ...
    for src in ky:
        ...
        if src["needs_clinician"].strip():
            row["needs_clinician"] = (
                "INHERITED — restate in French terms during the domain pass"
            )
        ...
        if cid in collapsed or cid in out_of_generation:
            ...
            if person == "first":
                ...
                if not candidate:
                    note = (
                        "no v1 French phrase exists: EX31 is the row the speaker "
            ...
            if drafted and (person == "first") == applies_first.get(cid, True):
                ...
                if person == "third":
                    note += (
                        ". Written in carer voice already; needs {REL} substituted "
...
def merge(fresh: list[dict], existing: list[dict]) -> tuple[list[dict], list[str]]:
    ...
    for row in fresh:
        ...
        if old.get("agreement_check", "") != expected:
            drift.append(
                f"{row['concept_id']} {row['person']} agreement_check: "

...
from english_relations import (
    ADULT_RELATIONS_EN,
    ALL_RELATIONS,
    CHILD_RELATIONS_EN,
    DOMAIN_RELATIONS_EN,
    HOUSEHOLD_RELATIONS_EN,
    NO_RELATIONS_EN,
    OBSTETRIC_RELATIONS_NO_MOTHER,
    PENDING_RULINGS,
)
...
REGENERATED = [
    c for c in COLUMNS if c not in AUTHORED and c not in ("concept_id", "person")
]
...
PERSON_NOTE = {
    "": "",
    "first = the pregnant woman; third = husband, mother or neighbour reporting": "First person is the pregnant woman herself. Third person is someone "
    "else reporting for her — her husband, her mother, a neighbour.",
    "usually third (the parent speaks); write first only if an older child would say it": "Usually the parent speaks, so the third person is the main row. Write "
    ...
    "version is the third person": "Usually the parent speaks. An older child could say the first person "
    "about themselves; the parent's version is the third person.",
    "usually third (the parent speaks); the parent speaks": "The parent speaks. The third person is the parent's report.",
    ...
    "the evidence the two were one concept.": "No first person: an unconscious patient cannot report their own "
    ...
    "the evidence the two were one concept.": "No first person: a convulsing patient cannot report it while it is "
    ...
    "untouched; NO_RELATIONS is a restriction, not a deletion.": "No third person: nobody comes in on someone else's behalf to collect a "
    ...
    "First person untouched.": "No third person: an adult asks for this service for themselves and "
...
APPLIES_NO_REASON = {
    ...
    (
        "PA04",
        "first",
    ): "Ruled out: a child in severe respiratory distress cannot speak for themselves.",
    (
        "PA05",
        "first",
...
LIFTED_HOLDS = {
    ...
    ("IF04", "first"): "the Kinyarwanda hold is on one unvalidated word for sweating.",
...
CLINICIAN: dict[str, tuple[bool, str]] = {
    "CR04": (
        True,
        "For a clinician: lower chest indrawing can be described as "
    ...
    "CC01": (
        True,
        "For a clinician: can a patient this ill still report their "
        "own drowsiness and deep breathing? This row is held.",
    ),
    "CC02": (
        True,
        "For a clinician: can a patient with low blood sugar report "
        "their own confusion? This row is held.",
    ),
    "HT03": (
        True,
        "For a clinician: can a patient concussed enough to be "
        "vomiting and confused give this account? This row is held.",
    ),
    "HT05": (
        True,
        "AXIS, AND IT MATTERS FOR THE WORDING. The concept is a limb "
    ...
    "EX27": (
        False,
        "The Kinyarwanda flag is about inflecting one verb for "
        "another person. It does not transfer.",
    ),
    "IF01": (
        False,
        "The Kinyarwanda flag is about one unvalidated word for a "
        "stiff neck. It does not transfer — but the sign is a "
        "danger sign, so keep the neck explicit.",
    ),
    "IF03": (
        False,
        "The Kinyarwanda flag is about wording. It does not "
    ...
    "IF04": (
        False,
        "The Kinyarwanda flag is about one unvalidated word for "
        "sweating. It does not transfer.",
    ),
    "IF06": (
        False,
        "The Kinyarwanda flag is about the register of one phrase "
    ...
    "NE04": (
        False,
        "Moot: this concept was collapsed and both persons are applies=no.",
    ),
    "NE05": (
        True,
        "OPEN QUESTION ABOUT THE CONCEPT, not about a language. The "
    ...
    "NE06": (
        True,
        "For a clinician: is a patient who can accurately report "
        "their own new confusion meaningfully confused? This row is "
        "held.",
    ),
    "OB03": (
        True,
        "For a clinician: this is a specific obstetric emergency "
    ...
    "OB05": (
        True,
        "For a clinician: the description of infected discharge "
    ...
    "EX43": (
        True,
        "For a clinician: is 'high fever and refusing to eat' URGENT, "
...
def ex_to_v1_english(v1) -> dict[str, str]:
    ...
    existing = [
        r
        for r in csv.DictReader(SPINE_V1.open(encoding="utf-8"))
...
def applies_no_reason(
    ...
    if concept_id in collapsed:
        return (
            f"This whole concept is out of generation — it was collapsed into "
...
def build() -> list[dict]:
    ...
    for src in spine:
        ...
        applies = src["applies"]
        held = (src["hold"] == "yes" and (cid, person) not in LIFTED_HOLDS) or (
            cid,
            person,
        ) in ADDED_HOLDS
        ...
        if applies == "no":
            notes.append(
                applies_no_reason(cid, person, person_note, collapsed, absorbed_by)
            )
        elif held:
            notes.append(HOLD_REASON[cid])
        elif src["hold"] == "yes":
            notes.append(
                "The Kinyarwanda row is held; the hold is LIFTED here "
        ...
        if src["needs_clinician"].strip():
            ...
            if applies != "no" and question and question not in notes:
                notes.append(
                    question if transfers else f"Not a question for you: {question}"
                )
        ...
        if is_ex and cid not in EX_GLOSS_OVERRIDE:
            notes.append(
                "Gloss is the frozen v1 English at this corpus position. "
...
RELATION_NOTE = {
    "My neighbour": "OPEN, AND WE WOULD LIKE YOUR VIEW. This word is gender-neutral in both "
    ...
    "My grandmother": "The Kinyarwanda word is a general term for an elderly woman rather than "
    ...
    "My neighbour's child": "Used only where a neighbour brings a child in. If that is not a natural "
...
def relation_sheet_rows() -> list[dict]:
    ...
    order = list(ALL_RELATIONS) + [
        m for m in CHILD_RELATIONS_EN if m not in ALL_RELATIONS
    ]
    return [
        {
            "english": member,
...
def assert_no_swahili(rows: list[dict], v1) -> None:
    ...
    swahili = {
        p.strip()
        for urgency in v1.SYMPTOMS["swahili"].values()
        for phrases in urgency.values()
        for p in phrases
    }
    v1_brief = ROOT / "review" / "speaker_brief_swahili.csv"
    if v1_brief.exists():
        swahili |= {
            r["current_swahili_phrase"].strip()
...
def main() -> int:
    ...
    if OUT.exists():
        ...
        if header == COLUMNS:
            ...
            authored = sum(
                1 for r in existing if (r.get("your_phrasing") or "").strip()
            )
            if authored:
                raise SystemExit(
    ...
    if replaced_stale:
        print(
            f"REPLACED {OUT.name}: the file on disk was the generator-era brief "
    ...
    print(f"  {len(LIFTED_HOLDS)} Kinyarwanda holds lifted, {len(ADDED_HOLDS)} added")
    print(f"  open questions carried on {questions}")
    print(
        f"wrote {RELATIONS_OUT.relative_to(ROOT)}: "

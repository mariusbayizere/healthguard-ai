"""FROZEN v1 vocabulary. Do not edit.

This is the exact phrase and frame inventory the v1 corpus was generated from,
kept live so `make verify-full` can keep re-deriving v1's 1,000,000 rows from
seed 42 and matching the frozen digests. Without it the CI reproducibility badge
and the paper's reproducibility claim would rest on git history rather than on
anything HEAD can prove.

It is a snapshot, not a shared base: `dataset/vocabulary.py` moved on at the v2
freeze (2026-09-05) to a monolingual Kinyarwanda inventory of 165 speaker-authored
phrases, and the two are deliberately independent from that point.

Select it with `generate_large_dataset.py --corpus-version 1`. Nothing imports it
implicitly.

Four languages, 46 phrases each, six mixed pairs, and every v2-only mechanism
empty - which is what "empty means behave exactly as v1 does" meant while both
lived in one file.
"""

...
def use_corpus_version(version: int) -> None:
    """Split v1's corpus with v1's inventory, or v2's with v2's.

    The partition is computed FROM THE PHRASE INVENTORY, not from the rows, so
    splitting v1's corpus while dataset/vocabulary.py holds v2's phrases produces
    a different partition and different digests - which is exactly what
    `make verify-full` caught at the v2 freeze. The corpus regenerated byte for
    byte and all four split digests moved.

    Call it before phrase_components() or split(). v2 is the default.
    """
    global PHRASE_CONCEPTS, PHRASE_VARIANTS, GROUPED_CONCEPTS
    if version == 2:
    ...
    GROUPED_CONCEPTS = V1.GROUPED_CONCEPTS

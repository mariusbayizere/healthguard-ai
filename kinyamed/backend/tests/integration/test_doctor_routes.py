...
def test_email_is_validated_and_normalised(client):
    assert (
        client.post(
            "/api/v1/doctors", json={"name": "Dr X", "email": "not-an-email"}
        ).status_code
        == 422
    )
    created = client.post(
        "/api/v1/doctors", json={"name": "Dr X", "email": " DR.X@Kinyamed.RW "}
    )
...
def test_patch_to_a_taken_email_returns_409(client, doctor_factory):
    ...
    response = client.patch(
        f"/api/v1/doctors/{second['id']}", json={"email": first["email"]}
    )
    assert response.status_code == 409, (
        "the old code raised an unhandled IntegrityError here"
    )
...
def test_patch_preserves_omitted_fields(client, doctor_factory):
    ...
    response = client.patch(
        f"/api/v1/doctors/{doctor['id']}", json={"is_on_duty": False}
    )
...
def test_doctor_with_consultations_cannot_be_deleted(
    client, doctor_factory, patient_factory, db
):
    ...
    db.add(
        Consultation(queue_entry_id=entry["id"], doctor_id=doctor["id"], notes="seen")
    )
    db.commit()

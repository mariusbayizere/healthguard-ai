...
def test_status_transition_is_validated(client, patient_factory):
    ...
    finished = client.patch(
        f"/api/v1/queue/{entry['id']}/status", json={"status": "DONE"}
    )
    assert finished.status_code == 200
    assert finished.json()["completed_at"] is not None

    reopened = client.patch(
        f"/api/v1/queue/{entry['id']}/status", json={"status": "WAITING"}
    )
...
def test_unknown_status_is_rejected(client, patient_factory):
    ...
    assert (
        client.patch(
            f"/api/v1/queue/{entry['id']}/status", json={"status": "NONSENSE"}
        ).status_code
        == 422
    )
...
def test_assigning_an_off_duty_doctor_is_refused(
...
def test_removal_preserves_clinical_history(client, patient_factory, db):
    ...
    assert db.query(TriageResult).count() == 1, (
        "cancelling must not delete the triage record"
    )
...
def test_queue_read_does_not_issue_a_query_per_row(client, patient_factory):
    ...
    assert len(selects) <= 4, (
        f"expected a constant number of queries, got {len(selects)}"
    )

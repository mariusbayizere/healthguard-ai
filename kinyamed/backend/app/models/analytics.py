...
class Analytics(TimestampedModel):
    ...
    __table_args__ = (
        CheckConstraint(
            "avg_wait_time_mins >= 0", name="ck_analytics_avg_wait_non_negative"
        ),
    )
    ...
    total_triaged: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )
    critical_cases: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )
    urgent_cases: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )
    routine_cases: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )

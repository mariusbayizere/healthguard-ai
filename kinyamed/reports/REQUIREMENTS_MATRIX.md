...
| FR-03-03 | Charts: bar, donut, pie, 7×24 heatmap, word cloud; shared date picker | M | **PARTIAL (backend) / MISSING (frontend)** | Backend only: urgency breakdown `routes/v1/analytics.py:32`, language breakdown `:48`. **Corrected 2026-09-17:** this row previously cited `routes/Dashboard.tsx` and `components/charts.tsx` as evidence; neither file exists. The frontend has four routes (`Login`, `Triage`, `Queue`, `Doctor`) and no admin or analytics page at all | No frontend; no heatmap; no per day/week/month grouping; `top_symptom` deliberately `None` (`services/analytics_service.py:123`) | Low |
...
| FR-03-07 | Configurable confidence threshold, audited | S | **INCORRECT (not missing)** | `MODEL_CONFIDENCE_THRESHOLD` at `core/config.py:109` is **live**, read at `services/review.py:39` and `services/queue_service.py:65`, and evaluated on read so a restart with a new value reclassifies existing rows; 6 tests in `test_review_threshold.py` (ran, pass). **Corrected 2026-09-17:** this row previously said "referenced by 0 lines of app code" and cited line 101; both were wrong | Not runtime-configurable (process-start env var only), so "effective within 60 s without restart" fails; no admin endpoint, so the change cannot be attributed | Med (L4) |
...
| FR-03-09 | Full audit log of every write | M | **PARTIAL** (was MISSING until 2026-09-17) | `audit_logs` table (`migrations/versions/c1f4a2b7d903`), `services/audit.py`, writes in all 23 state-changing endpoints. `tests/integration/test_audit_log.py` enumerates the app's own route table and asserts **exactly one** row per operation, plus PII scrubbing and roll-back atomicity (29 tests, ran, pass) | No read endpoint yet, so "searchable by date, user, action" is unbuilt; failed logins deliberately not audited (they change no state, and an unauthenticated caller must not be able to append rows) | Med — accountability now recorded, not yet queryable |
